...
%       xmin = NEWTON(f,2,[0,0],'alpha',1e-4,'epsilon',1e-9)
...
while sqrt(sum(delt.^2)) > epsilon && iter < maxiter
    % Step in negative direction based on gradient and hessian
    delt = alpha*grad(f,xmin)/hessian(f,xmin);
    xmin = xmin - delt;
